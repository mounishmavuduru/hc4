...
2026-08-08/10; a novelty search (documented in \texttt{status.md} and the
...
$t^2\partial_t^2L=0$, so $L$ is affine in $t$ along every ray. Hence
$L=A+\widetilde B$ with $A,\widetilde B$ holomorphic and homogeneous of
Euler-degree $0$ and $1$, and since any analytic continuation of $L$ solves
the same equation, this expression \emph{is} the continuation of $L$ over
the whole cone. As $\nabla A$ is homogeneous of degree $-1$ and
$\nabla\widetilde B$ of degree $0$,
\[
\psi(tp)=\nabla L(tp)=t^{-1}\nabla A(p)+\nabla\widetilde B(p)=:t^{-1}a+b .
\]
The identity $\varphi(\psi(q))=q$ continues along the cone as well, so
\[
\nabla e\bigl(t^{-1}a+b\bigr)=t\,p\qquad\text{for all }t .
\]
...
meet the branch locus). Three repaired proofs now stand, none continuing
$\psi$: the Euler-eigensplit extension (in
\texttt{theorem\_G\_paper.tex}), an elementary local-flow argument
($x''+x'=0$; \texttt{advG\_flow\_proof.py}), and a purely algebraic
Puiseux-series-at-infinity argument valid over any field of
characteristic $0$ (\texttt{pzG\_audit\_identities.py}).
\end{remark}
