...
\begin{remark}[scope of case (iii)]
Cases (i), (ii) and case (iii) for $\deg e_1\le2$ are unconditional (and
underlie the unconditional Theorem~\ref{thm:deg4} in degree $\le4$, which does
not use Theorem~\ref{thm:G}). Case (iii) in \emph{general} degree uses
Corollary~\ref{cor:E} to reduce a degree-$\ge3$ pivot coefficient $e_1$ to an
affinely $2$-variable form (without it $\det\Hess f$ need not even be affine in
$x_4$: e.g.\ $e_1=x_1^2+x_2x_3$ gives $x_4$-degree $2$). Corollary~\ref{cor:E}
rests on the rigidity Theorem~\ref{thm:G}; the all-degree equivalence therefore
inherits Theorem~\ref{thm:G}'s standing (whose original proof was withdrawn and
is now carried by the repaired proofs cited there). So the all-degree
``$\HC_4$-with-a-pivot $\Leftrightarrow\JC_2$'' is conditional on
Theorem~\ref{thm:G}; the $\deg e_1\le2$ and degree-$\le4$ statements are not.
\end{remark}
