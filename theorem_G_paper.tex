...
affine-differential-geometry communities. Revision of August 25, 2026:
incorporates the third independent audit of Theorem~\ref{thm:G}
(certificates \texttt{atkG\_*.py}), whose derivation-theoretic proof is
now the second proof below.}}
\date{August 25, 2026}
...
(Theorem~\ref{thm:G}); the converse is false. Three independent proofs
are given: one analytic (through the local Legendre transform and an Euler
ordinary differential equation on the target), and two purely algebraic
(one through a derivation of the rational function field, one through
Puiseux series at infinity), the algebraic ones valid over every field of
...
machine certificate (Appendix~\ref{sec:verification}); the three proofs of
...
Finally, Meng's doubling \cite[Prop.~1.4]{Meng2006} (stated there for
$\Jac F=1$; the general block computation is
...
We give three independent proofs. The first is analytic and exhibits the
mechanism (the inverse gradient would have to be affine in $t^{-1}$
along rays of the target). The second is purely algebraic and short: it
works in the rational function field with the derivation along the
vector field $(\Hess e)^{-1}\nabla e$ and needs no auxiliary objects. The
third is also algebraic and transports the Legendre computation of the
first proof to Puiseux series at infinity, making the relation between
the first two visible. The second and third proofs are valid over any
field of characteristic zero. All three begin with the same reduction.
...
for the referee's scrutiny, and we supply two independent algebraic
proofs that use no function theory at all.
\end{remark}

\begin{proof}[Second proof of Theorem~\ref{thm:G} (algebraic, via a
derivation)]
Keep the common set-up ($n\ge2$, $B\equiv0$, $D\not\equiv0$) and work in
the rational function field $K:=\C(x_1,\dots,x_n)$, over which $H$ is
invertible because $D\ne0$ in $K$. Put
\[
V:=H^{-1}g\in K^n,\qquad \delta:=\sum_{j=1}^nV_j\,\partial_j ,
\]
so that $\delta$ is a $\C$-linear derivation of $K$ with
$\delta(x_j)=V_j$ and $\delta(P)=\langle\nabla P,V\rangle$ for $P\in K$.
For any derivation $\partial$ of $K$ write
$\partial x:=(\partial x_1,\dots,\partial x_n)^{\top}\in K^n$; thus
$\partial_jx=e_j$ and $\delta x=V$. Derivations act entrywise on
vectors and matrices over $K$.

\emph{Step 1 (three identities).} Let $\partial$ be any derivation of
$K$. Since $\partial_jg_i=H_{ij}$, the chain rule gives
$\partial g=H\,\partial x$, and differentiating $HH^{-1}=I$ gives
$\partial(H^{-1})=-H^{-1}(\partial H)H^{-1}$. Hence
\begin{equation}\label{eq:dV}
\partial V=\partial(H^{-1}g)=-H^{-1}(\partial H)V+H^{-1}H\,\partial x
=\partial x-H^{-1}(\partial H)\,V .
\end{equation}
For $\partial=\delta$ the chain rule reads
\begin{equation}\label{eq:dg}
\delta g=HV=g .
\end{equation}
Next, $B=g^{\top}\adj(H)g=D\,g^{\top}V$, so $B\equiv0$ and $D\ne0$ give
$g^{\top}V=0$ in $K$. Applying $\partial_j$, using $\partial_jg=He_j$ and
\eqref{eq:dV} with $\partial=\partial_j$,
\[
0=\partial_j\bigl(g^{\top}V\bigr)
=e_j^{\top}HV+g^{\top}e_j-g^{\top}H^{-1}(\partial_jH)V
=2g_j-V^{\top}(\partial_jH)V .
\]
Let $M\in K^{n\times n}$ be the symmetric matrix
$M_{ik}:=\sum_lV_l\,\partial_i\partial_k\partial_le$, i.e.\ $M=\delta H$.
By symmetry of the third derivatives,
$V^{\top}(\partial_jH)V=\sum_{i,k}V_i\,\partial_i\partial_k\partial_je\,V_k
=(MV)_j$, so
\begin{equation}\label{eq:MV}
MV=2g .
\end{equation}
Finally, \eqref{eq:dV} with $\partial=\delta$ (where $\delta H=M$ and
$\delta x=V$) together with \eqref{eq:MV} gives
\begin{equation}\label{eq:dVV}
\delta V=V-H^{-1}MV=V-2H^{-1}g=-V .
\end{equation}

\emph{Step 2 (eigenvectors of $\delta$).} Put $a:=V$ and $b:=x+V$ in
$K^n$. By \eqref{eq:dVV}, $\delta a=-a$ and $\delta b=V+\delta V=0$: the
coordinates of $b$ are constants of $\delta$, those of $a$ are
eigenvectors of eigenvalue $-1$. Let $d:=\deg e$; then $d\ge2$ (as
$D\ne0$) and each $g_i$ is a polynomial of degree $\le d-1$. Taylor's
formula for polynomials in characteristic $0$,
$P(y-z)=\sum_\alpha(\partial^\alpha P)(y)\,(-z)^\alpha/\alpha!$ (a finite
sum), evaluated at $y=b$, $z=a$, gives
\[
g_i=g_i(b-a)=\sum_{k=0}^{d-1}c_{i,k},\qquad
c_{i,k}:=\sum_{|\alpha|=k}\frac{(-1)^k}{\alpha!}\,
(\partial^\alpha g_i)(b)\,a^\alpha\in K .
\]
Since $\delta$ kills every coordinate of $b$,
$\delta\bigl((\partial^\alpha g_i)(b)\bigr)
=\sum_j(\partial_j\partial^\alpha g_i)(b)\,\delta b_j=0$; and
$\delta(a^\alpha)=\sum_j\alpha_j\,a^{\alpha-e_j}\,\delta a_j
=-|\alpha|\,a^\alpha$. Hence
\[
\delta c_{i,k}=-k\,c_{i,k}\qquad(0\le k\le d-1).
\]

\emph{Step 3 (contradiction).} By \eqref{eq:dg},
$\sum_kc_{i,k}=g_i=\delta g_i=\sum_k(-k)\,c_{i,k}$, that is,
\[
\sum_{k=0}^{d-1}(k+1)\,c_{i,k}=0 .
\]
Applying $\delta^m$ for $m=0,\dots,d-1$ gives
$\sum_k(-k)^m(k+1)\,c_{i,k}=0$. The Vandermonde matrix
$\bigl((-k)^m\bigr)_{0\le m,k\le d-1}$ with the distinct nodes
$0,-1,\dots,-(d-1)$ is invertible, so $(k+1)c_{i,k}=0$ for every $k$, and
in characteristic $0$ every $c_{i,k}=0$. Thus $g_i=\sum_kc_{i,k}=0$ for
all $i$, i.e.\ $g\equiv0$; then $H\equiv0$ and $D\equiv0$, contradicting
$D\not\equiv0$.
\end{proof}

\begin{remark}[reading of the second proof]\label{rem:flow}
Identity \eqref{eq:dg} says that $g$ is an eigenvector of $\delta$ with
eigenvalue $+1$, while Step~2 exhibits each $g_i$ as a \emph{finite} sum
of eigenvectors with eigenvalues $0,-1,\dots,-(d-1)$; polynomiality of
$e$ enters exactly in this finiteness. For the rational function
$e=x_2/x_1$ (which has $B\equiv0$ and $\det\Hess e=-x_1^{-4}$) one finds
$V=-x$, $b=0$, $a=-x$, and $g(b-a)=g(x)$ is homogeneous of degree $-1$ in
$a$: eigenvalue $+1$, which a polynomial $g$ can never produce. The proof
is the algebraic core of an ordinary-differential-equation argument found
in the first independent audit of the project: on $\{D\ne0\}$,
\eqref{eq:dVV} says that the local holomorphic flow $x'(s)=V(x(s))$,
$x(0)=x_0$, satisfies $x''=-x'$, so $x(s)=a_0e^{-s}+b_0$, while
$\frac{d}{ds}\nabla e(x(s))=HV=g$ gives $\nabla e(x(s))=e^{s}p_0$; the
polynomial vector $\tau\,\nabla e(a_0\tau+b_0)-p_0$ then vanishes at the
infinitely many values $\tau=e^{-s}$ but not at $\tau=0$. The derivation
$\delta$ is the infinitesimal generator of that flow, and the eigenvalue
bookkeeping replaces the exponential. The identities \eqref{eq:dV},
\eqref{eq:MV} are certified fully generically at the jet level in
\texttt{advG\_flow\_proof.py}, \texttt{atkG\_algebraic\_proof.py} and
\texttt{rwG\_paper\_certificate.py} (Appendix~\ref{sec:verification}).
\end{remark}

\begin{proof}[Third proof of Theorem~\ref{thm:G} (algebraic, via Puiseux
series at infinity)]
Keep the common set-up ($n\ge2$, $B\equiv0$, $D\not\equiv0$).
...
\begin{remark}[one skeleton, three proofs]\label{rem:skeleton}
The three proofs share one skeleton: along the orbit of the field
$(\Hess e)^{-1}\nabla e$ (a ray $t\mapsto tp_0$ on the target side) the
inverse gradient is affine in the reciprocal ray parameter,
$\psi(tp_0)=t^{-1}a+b$, which a polynomial gradient cannot match. In the
first proof this is Step~7, in the second it is $x=b-a$ with
$\delta b=0$, $\delta a=-a$, in the third it is $\chi=t^{-1}a+b$. The
hypothesis $B\equiv0$ enters, respectively, as $(\Eu^2-\Eu)L=0$, as
$g^{\top}V=0$, and as $\partial_t^2\Lambda=0$.
\end{remark}
...
conditions of Step~1 of the first proof ($D(x_0)\ne0$,
$\nabla e(x_0)\ne0$) are open conditions automatically satisfiable under
the contradiction hypothesis $D\not\equiv0$; the second and third proofs
need no base point at all. The first proof uses the holomorphic inverse
function theorem and the identity theorem and is carried out over $\C$;
the second uses only the calculus of derivations of a field; the third
uses field theory (Newton--Puiseux and the extension theorems for
embeddings and derivations). Polynomiality of $e$ is used exactly once in
each proof: in the last step of the first and third, and in the
finiteness of the Taylor expansion in Step~2 of the second.
\end{remark}

\begin{remark}[other fields]\label{rem:fields}
The second and third proofs are valid verbatim over any field $k$ of
characteristic $0$ (in the third, only $\overline\FF$ needs to be
algebraically closed, and it is).
...
(ii) Polynomiality is used exactly once in each proof
(Remark~\ref{rem:hyp}), and cannot be dropped: the algebraic function
...
argument dropped the linear terms of $e$ --- on which $B$ depends, e.g.\
$B(\tfrac12x_1^2+x_2)=1$ but $B(\tfrac12x_1^2)=0$ --- and was not
exhaustive as first claimed; the full-coefficient-space decisions, linear
terms included, are \texttt{pzG\_n2\_decision.py} for degrees $\le4$ and
\texttt{atkG\_n2\_decision.py} for degrees $\le5$, both by radical
membership over the whole coefficient ring.)
(iv) The certificates \texttt{theorem\_G.py}, \texttt{advG\_flow\_proof.py},
\texttt{pzG\_audit\_identities.py} and
\texttt{atkG\_algebraic\_proof.py} verify the structural identities of the
three proofs fully symbolically (at the jet level, which suffices since
every identity involved depends only on the $3$-jet of $e$ at a point)
and test the conclusion decisively on complete low-degree ans\"atze.
These are consistency certificates for hand proofs, not machine proofs
of the theorem.
\end{remark}
...
\cite[Thm.~3.3]{dBvdE} (restated as \cite[Thm.~2.4]{deBondt2015} for
polynomials without terms of degree $<2$, which is all that matters for
$\Hess$): after a linear change such a polynomial is either in
$\C[x_1,x_2]+\C x_3$ or of the form $g_1(x_1)+g_2(x_1)x_2+g_3(x_1)x_3$,
and in the second family one computes
...
review}): Theorem~\ref{thm:G} (with its three proofs; the second and
third were found in the project's independent audits);
...
$s$-linear extraction); \texttt{rwG\_paper\_certificate.py} (the
certificate of the present revision: an independent structural
compile-check, an undefined-macro scan, and exact re-derivations of the
identities \eqref{eq:dV}, \eqref{eq:dg}, \eqref{eq:MV},
\eqref{eq:dVV} of the second proof --- in polynomial form,
$D\,\partial_jW-W\,\partial_jD=D^2e_j-\adj(H)(\partial_jH)W$ and
$D\,\nabla B-B\,\nabla D=2D^2g-\widetilde MW$ with $W=\adj(H)g=DV$,
$\widetilde M=\sum_l(\partial_lH)W_l=DM$, for fully generic $3$-jets in
$n=2,3$, which is a proof for all $e$ in those dimensions --- the
eigenvalue bookkeeping of its Step~2 for a generic polynomial, the
Vandermonde step, and the witnesses of Remark~\ref{rem:flow});
\texttt{theorem\_G.py}, \texttt{advG\_flow\_proof.py},
\texttt{pzG\_audit\_identities.py}, \texttt{atkG\_algebraic\_proof.py}
(structural identities of the three proofs of Theorem~\ref{thm:G}, the
last also for fully generic $3$-jets in $n=4$; low-degree conclusion
tests; the Puiseux mechanism on an exact branch; the sharpness witnesses
of Remark~\ref{rem:flow}); \texttt{pzG\_n2\_decision.py} (degrees
$\le4$) and \texttt{atkG\_n2\_decision.py} (degrees $\le5$), the complete
decisions of the $n=2$ case over the full coefficient space, linear terms
included (the earlier \texttt{theorem\_G\_n2\_deg45.py} supplies $180$
exact instances of degrees $4$--$6$ with $\det\Hess\ne0$, all with
$B\ne0$, but its ``complete slice'' is the homogeneous slice, where the
theorem is a one-line consequence of Lemma~\ref{lem:identE}, so it is not
an exhaustive check and is not cited as one);
...
property}, J. Algebra \textbf{577} (2021), 175--202,
doi:10.1016/j.jalgebra.2021.01.037; arXiv:1904.01800, \S2
(``Homogeneous polynomials''), Proposition~2.3 (source label
\texttt{identity1}).
...
Nachr. \textbf{290} (2017), no.~2--3, 293--320; arXiv:1503.09108.
